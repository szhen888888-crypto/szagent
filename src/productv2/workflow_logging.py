...
EVENT_LABELS = {
    ...
    "ai_call_lock_acquired": "AI 调用缓存占用",
    "ai_call_lock_wait": "AI 调用缓存等待",
    "ai_call_lock_hit": "AI 调用缓存命中",
    "ai_call_lock_saved": "AI 调用缓存保存",
}

NODE_DESCRIPTIONS = {
    "load_candidates": "扫描当前输入状态，从显式 JSON 或 SQLite 中加载候选商品，并选择一个有平台适配器的可处理商品。",
    "merge_main_images": "调用平台适配器获取商品主图，下载可用图片并合并为带编号的临时拼图。",
    "detect_size_reference": "调用视觉 LLM 检查编号拼图，执行产品合格性检测；当前硬规则是必须存在可判断尺寸比例的真人或人体参照，并确定尺寸参考图和产品主图编号。",
    "select_enroute_reference": "根据当前商品类目查询 SQLite Enroute 学习表和已逆向缓存，并决定本轮需要学习的同类目参考图。",
    "learn_enroute_profiles": "按学习计划调用 LLM 逆向同类目 Enroute 02.jpg 并写入 profile 缓存。",
    "select_wearing_style_profile": "用当前产品主图、尺寸参考图、Enroute profile 摘要和固定模特摘要选择风格 profile 与模特 profile。",
    "compile_wearing_generation_prompt": "加载实际 Enroute profile、固定模特 profile、产品主图和尺寸图，编排最终图片生成 prompt。",
    "generate_wearing_image": "使用已编译的 prompt 和输入图片调用图片生成接口，并把生成结果保存到产品产物目录。",
