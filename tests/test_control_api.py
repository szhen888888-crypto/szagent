...
def test_control_api_retries_compile_prompt_node(monkeypatch) -> None:
    posts = []
    ...
    client = TestClient(control_api.app)
    response = client.post(
        "/api/threads/thread-1/retry-node",
        json={
            "api_url": "http://testserver",
    ...
    assert response.status_code == 200
    assert response.json()["mode"] == "retry_node"
    assert response.json()["node"] == "compile_wearing_generation_prompt"
    assert posts == [
        (
            "http://testserver",
            "/threads/thread-1/state",
