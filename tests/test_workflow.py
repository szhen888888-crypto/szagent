...
def test_recompile_prompt_review_action_skips_prompt_checkpoint(
    monkeypatch,
    tmp_path,
) -> None:
    ...
    Image.new("RGB", (10, 10), "blue").save(model)
    calls = {"count": 0}

    def fake_compile_wearing_generation_prompt(*_args, **_kwargs):
        calls["count"] += 1
        return {
            "status": "ok",
            "reason": "wearing_generation_prompt_compiled",
            "prompt": f"compiled prompt {calls['count']}",
    ...
    monkeypatch.setattr(
        graph_module,
        "compile_wearing_generation_prompt",
        fake_compile_wearing_generation_prompt,
    )
    state = {
        "selected_product": {
            "id": 1,
        ...
        "wearing_style_selection_result": {
            ...
            "enroute_product_id": "necklaces:ref-1",
            "category": "necklaces",
            "summary": "适合长链。",
            "analysis": {"shooting_style": {"shot_type": "upper body"}},
            "selected_model_profile": {
                "profile_key": "romantic_rebel",
                "name": "Romantic Rebel",
                "image_path": str(model),
            },
            "selection": {"reason": "测试选择"},
        },
    }

    first = graph_module._compile_wearing_generation_prompt(state)
    second = graph_module._compile_wearing_generation_prompt(
        {
            **state,
            **first,
            "manual_review_decision": {"action": "recompile_prompt"},
    ...
    assert first["wearing_generation_prompt_result"]["prompt"] == "compiled prompt 1"
    assert second["wearing_generation_prompt_result"]["prompt"] == "compiled prompt 2"
    assert (
        second["ai_checkpoints"]["compile_wearing_generation_prompt"]["attempt_count"]
        == 2
    )


def test_recompile_prompt_review_action_regenerates_wearing_image(
    monkeypatch,
    tmp_path,
) -> None:
    previous = tmp_path / "wearing_image_attempt_1.png"
    generated = tmp_path / "wearing_image_attempt_2.png"
    Image.new("RGB", (10, 10), "green").save(previous)
    Image.new("RGB", (10, 10), "purple").save(generated)
    calls = {"count": 0}
    ...
    state = {
        ...
        "wearing_image_result": {
            "status": "ok",
            "generated_image_path": str(previous),
            "attempt": 1,
        },
    }

    output = graph_module._generate_wearing_image(state)

    assert calls["count"] == 1
    assert output["wearing_generation_attempt"] == 2
    assert output["wearing_image_result"]["generated_image_path"] == str(generated)
    assert output["manual_review_decision"] == {}
