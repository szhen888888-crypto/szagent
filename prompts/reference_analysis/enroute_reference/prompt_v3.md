...
- 不得输出 Markdown。
- 不得输出解释。
- 输出必须是合法 JSON，可被直接 JSON 解析，不得用 ``` 代码块包裹。
- 字段名、层级和结构必须与下面给出的 JSON 结构完全一致；不要新增、改名或删除字段。
...
- 固定妆容政策的具体内容以输出 JSON 中 retouching_and_makeup_policy.fixed_default_makeup_rule 字段为唯一来源，本段不再重复正文，只需保证你填入该字段的内容与之一致。
