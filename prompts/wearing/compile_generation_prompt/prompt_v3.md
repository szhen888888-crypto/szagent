...
- 总长度控制在 900-1600 个英文单词或 1200-2600 个中文字符以内；宁可少而准，不要长而散。
- 输出应像给无状态 AI 绘图软件的明确制图 brief，而不是工作流日志。

凝练规则：
- 你可以在内部参考 Enroute 逆向 profile 的 `model_style`、`clothing_style`、`scene_style`、`shooting_style`、`camera_estimate`、`lighting_estimate`、`pose_estimate` 等维度，但最终只保留对图片生成有直接作用的信息。
- 删除弱相关、重复、抽象和无法执行的信息。
...
6. 姿势构图：只保留一个明确构图方案，包括景别、裁切范围、头部/肩颈/躯干角度、手是否入镜、产品在画面中的位置。
7. 服装露肤：服装和领口必须服务于展示首饰，不遮挡关键产品长度或主体。
8. 摄影参数：给出少量真正有用的相机参数，例如画幅、焦距等效、相机距离、机位高度、对焦点、景深。
9. 光线背景后期：明确光线方向、阴影、高光、背景简洁度、肤质和产品边缘清晰度。
10. 负面约束：只保留最高风险项，例如 wrong product、extra jewelry、wrong length、copied Enroute jewelry、covered pendant、changed model identity、over-retouched skin、logo、watermark。

硬规则：
- 最终 prompt 中必须反复强调：image 01 is the product source，image 02 is the scale/fit source，fixed model image is identity source。
...
[user]
请把材料凝练成一段可直接交给图片生成模型的最终 prompt。只输出 prompt 正文，不要 JSON，不要 Markdown，不要解释。
